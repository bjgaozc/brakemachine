...
var W;
var windowapi;
try {windowapi = frameElement?frameElement.api:undefined, W = windowapi?windowapi.opener:undefined;} catch (e) {}//内容页中调用窗口实例对象接口
var win_auto_max = false;
 ...
 * @param height 编辑框的高度
 * @param key 表的主键字段
 * @param type 表的类型，默认datagrid
 */
function update(title,url,id,width,height,key,type) {
	gridname=id;
	var rowsData = null;
	if(!type){
		rowsData = $('#'+id).datagrid('getSelections');
	}else if("treegrid"==type){
		rowsData = $('#'+id).treegrid('getSelections');
	}
...
function deleteALLSelect(title,url,gname) {
    ...
    if (rows.length > 0) {
    	$.messager.confirm('消息','你确定永久删除该数据吗?', function(r) {
		   if (r) {
...
function exit(url, content) {
	try{
		$.messager.confirm('消息',content,function(r){
	...
	if(indexStyle=='sliding'||indexStyle=='bootstrap'||indexStyle=='ace'){
		...
		if (window.top.$('#maintabs').tabs('exists', subtitle)) {
			...
			if (url.indexOf('isHref') != -1) {
				...
				var h = (window.top.$('#maintabs .tabs-panels.tabs-panels-noborder').height()-4) + "px";
				window.top.$('#maintabs').tabs('add', {
					title : subtitle,
					content : '<iframe src="' + url + '" frameborder="0" style="border:0;width:100%;height:99.4%;min-height:'+h+';"></iframe>',
					closable : true,
					icon : icon
				});
			}
