...
function deleteALLSelect(title,url,gname) {
    ...
    if (rows.length > 0) {
    	$.messager.confirm('Message','Do you want to delete this data for ever?', function(r) {
		   if (r) {
...
function confuploadify(url, id) {
	$.messager.confirm('Message','Delete confirm', function(r){
		if(r) deluploadify(url, id);
	});
}
/**
 * 执行删除附件
...
function exit(url, content) {
	try{
		$.messager.confirm('Message',content,function(r){
