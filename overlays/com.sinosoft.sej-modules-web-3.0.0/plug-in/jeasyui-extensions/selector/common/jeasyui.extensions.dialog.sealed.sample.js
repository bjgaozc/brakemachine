/**
* 
* 由于使用了 lhgDialog 覆盖了 easyui 的 dialog ,所以，这里换成 easyui 的 window
* 
* jQuery EasyUI 1.4.3
    ...
    $.easyui.showDialog = function (options, frame) {
        ...
        opts.onBeforeClose = function () {
            ...
            var ret = $.isFunction(onBeforeClose)
                ...
                : undefined;
            return ($.isFunction($.fn.window.defaults.onBeforeClose) && $.fn.window.defaults.onBeforeClose.apply(this, arguments) == false)
                ? false
                : ret;
        };
        opts.onClose = function () {
            ...
            if ($.isFunction($.fn.window.defaults.onClose)) {
                $.fn.window.defaults.onClose.apply(this, arguments);
            }
            if (opts.autoDestroy) {
                $(this).window("destroy");
            }
        ...
        opts.onBeforeDestroy = function () {
            var ret = $.isFunction(onBeforeDestroy)
                ...
                : undefined;
            return ($.isFunction($.fn.window.defaults.onBeforeDestroy) && $.fn.window.defaults.onBeforeDestroy.apply(this, arguments) == false)
                ? false
                : ret;
        };
        opts.closed = true;
        ...
        var save =
            {
                ...
                handler: function (dia, btn) {
                    ...
                    $.util.delay(function () { $(dia).window("close"); });
                }
            },
            close = {
        ...
        var topts,
            ...
            toolbuttons;
        if (opts.href) {
            var onBeforeLoad = opts.onBeforeLoad || $.fn.window.defaults.onBeforeLoad,
                onLoad = opts.onLoad || $.fn.window.defaults.onLoad;
        ...
        topts = t.window(opts).window("options");
        var state = $.data(t[0], "window"),
            dialog = t.window("window"),
            buttonbar = dialog.children(".window-button");

        btns = buttonbar.children("a");
        toolbuttons = t.window("header").find(".panel-tool a");
