...
<script type="text/javascript">
	$(function() {
		$('#functionid').tree({
			...
			url : 'appCommonController.do?setAuthority&roleId=${roleId}',
		...
		$("#functionListPanel").panel(
				{
					...
					tools:[{iconCls:'fa fa-lg fa-save',handler:function(){mysubmit();}}]
				}
		);
		$("#operationListpanel").panel(
				{
					title :'<t:mutiLang langKey="operate.manage"/>',
					tools:[{iconCls:'fa fa-lg fa-save',handler:function(){submitOperation();}}]
				}
		);
			...
			$("#dataRuleListpanel").panel(
				{
					title :'数据规则权限',
					tools:[{iconCls:'fa fa-lg fa-save',handler:function(){submitDataRule();}}]
				}
		);
	});
	function mysubmit() {
